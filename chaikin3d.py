...
# how to plot
parser.add_argument(
    "-a", "--alpha", type=float, help="Alpha/Opacity value for mesh rendering (df. 0.8)"
)
parser.add_argument(
...
smc = (
    parse_bool(args["show_main_connections"]) if args["show_main_connections"] else True
)
sgc = (
    parse_bool(args["show_graphical_connections"])
    if args["show_graphical_connections"]
    else False
)
# how to plot
alpha = args["alpha"] if args["alpha"] else 0.8
...
main_conn_color = (
    args["main_connection_color"] if args["main_connection_color"] else "darkred"
)
graph_conn_color = (
    args["graphical_connection_color"]
    if args["graphical_connection_color"]
    else "black"
)
# other
...
if test:
    ...
    for invalid_option in invalid_options:
        if args[invalid_option] is not None:
            raise Exception(
                "Cannot use this option with '-t'/'--test': {}".format(invalid_option)
            )
...
def draw_full(renderer: Renderer, poly: Polyhedron) -> None:
    ...
    main_poly_dd = renderer.get_polyhedron_draw_data(
        poly, type_="main", alpha=0.6, color=polygon_color
    )
    graphical_poly_dd = renderer.get_polyhedron_draw_data(
        poly, type_="graphical", alpha=0.6, color=polygon_color
    )
    alpha_poly_dd = renderer.get_polyhedron_draw_data(
        poly, alpha=0.6, color=polygon_color
    )
...
def draw_chaikin_evolution(
    renderer: Renderer, poly: Polyhedron, n: int, coef: float, alpha: float = 0.8
) -> None:
    ...
    renderer.init_subplots(
        rows, cols, subplot_titles=["Chaikin Gen {}".format(i) for i in range(n + 1)]
    )
    for i in range(n + 1):
...
def main():
    ...
    if not input_file and not evaluation_string:
        raise Exception(
            'You have to give a polyhedron! Either through the "-i", "-e" or both options! (cf. README)'
        )
    ...
    if plot == "simple":
        poly_dd = renderer.get_polyhedron_draw_data(
            poly, type_="any", alpha=alpha, color=polygon_color
        )
        if smc:
            main_conn_dd = renderer.get_connections_draw_data(
                poly, type_="main", line_color=main_conn_color, node_color=node_color
