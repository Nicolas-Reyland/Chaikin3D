...
def generate_triangle_strip(width, length, offset=0.1):
    ...
    for line in triangle_strip:
        for node in line:
            node.connections = list(
                filter(lambda conn: conn.position, node.connections)
            )
...
def apply_random_3D(triangle_strip):
    ...
    for line in triangle_strip:
        for node in line:
            ...
            node.position[1] = random.uniform(-0.1, 0.1)

            # change node.connections position
            for conn in node.connections:
                if conn.position:
                    conn.position[1] = random.uniform(-0.1, 0.1)
...
class __vector__:
    def __init__(self):
        self.vector = lambda a, b: np.array(
            [b[i] - a[i] for i in range(len(a))]
        )  # replace 'np.array' with 'tuple' is something is broken now

        self.add = lambda vector1, vector2: [
            x + vector2[i] for i, x in enumerate(vector1)
        ]
        self.subtract = lambda vector1, vector2: [
            x - vector2[i] for i, x in enumerate(vector1)
        ]

        self.division = lambda vector1, vector2: [
            x / vector2[i] for i, x in enumerate(vector1)
        ]
        self.division_k = lambda vector, k: [x / k for x in vector]

        self.multiplication = lambda vector1, vector2: [
            x * vector2[i] for i, x in enumerate(vector1)
        ]
        self.multiplication_k = lambda vector, k: [x * k for x in vector]

        self.cross_product = lambda vector1, vector2: (
            vector1[1] * vector2[2] - vector1[2] * vector2[1],
            vector1[2] * vector2[0] - vector1[0] * vector2[2],
            vector1[0] * vector2[1] - vector1[1] * vector2[0],
        )
        self.dot_product = lambda vector1, vector2: sum(
            [vector1[i] * vector2[i] for i in range(len(vector1))]
        )
        ...
        self.angle = lambda vector1, vector2: np.arccos(
            self.dot_product(self.normalize(vector1), self.normalize(vector2))
        )
        self.angle_sign = (
            lambda base_vector, vector2: 1 if vector2[0] <= base_vector[0] else -1
        )

        self.rotate2d = lambda vector, teta: rotate2d(vector, (0, 0), teta)
        self.rotate3d_x = lambda vector, teta: transformation_matrix.apply_matrix(
            transformation_matrix.vector_rotate3d_x_axis(teta), vector
        )
        self.rotate3d_y = lambda vector, teta: transformation_matrix.apply_matrix(
            transformation_matrix.vector_rotate3d_y_axis(teta), vector
        )
        self.rotate3d_z = lambda vector, teta: transformation_matrix.apply_matrix(
            transformation_matrix.vector_rotate3d_z_axis(teta), vector
        )
        self.rotate3d_vector = (
            lambda vector1, vector2, teta: transformation_matrix.apply_matrix(
                transformation_matrix.vector_rotate3d_unit_vector(
                    self.normalize(vector2)[0],
                    self.normalize(vector2)[1],
                    self.normalize(vector2)[2],
                    teta,
                ),
                vector1,
            )
...
class __transformation_matrix__:
    def __init__(self):
        self.vector_rotate2d = lambda teta: [
            [np.cos(teta), -np.sin(teta)],
            [np.sin(teta), np.cos(teta)],
        ]
        self.vector_rotate3d_x_axis = lambda teta: [
            [1, 0, 0],
            [0, np.cos(teta), -np.sin(teta)],
            [0, np.sin(teta), np.cos(teta)],
        ]
        self.vector_rotate3d_y_axis = lambda teta: [
            [np.cos(teta), 0, np.sin(teta)],
            [0, 1, 0],
            [-np.sin(teta), 0, np.cos(teta)],
        ]
        self.vector_rotate3d_z_axis = lambda teta: [
            [np.cos(teta), -np.sin(teta), 0],
            [np.sin(teta), np.cos(teta), 0],
            [0, 0, 1],
        ]
        self.vector_rotate3d_unit_vector = lambda teta, ux, uy, uz: [
            [
                (1 - np.cos(teta)) * ux ** 2 + np.cos(teta),
                (1 - np.cos(teta)) * ux * uy - np.sin(teta) * uz,
                (1 - np.cos(teta)) * ux * uz + np.sin(teta) * uy,
            ],
            [
                (1 - np.cos(teta)) * ux * uy + np.sin(teta) * uz,
                (1 - np.cos(teta)) * uy ** 2 + np.cos(teta),
                (1 - np.cos(teta)) * uy * uz - np.sin(teta) * ux,
            ],
            [
                (1 - np.cos(teta)) * ux * uz - np.sin(teta) * uy,
                (1 - np.cos(teta)) * uy * uz + np.sin(teta) * ux,
                (1 - np.cos(teta)) * uz ** 2 + np.cos(teta),
            ],
        ]
        self.apply_matrix = lambda matrix, vector: [
            sum([matrix[a][b] * vector[b] for b in range(len(vector))])
...
distance3d = lambda a, b: np.sqrt(
    (b[0] - a[0]) ** 2 + (b[1] - a[1]) ** 2 + (b[2] - a[2]) ** 2
)
distance = lambda a, b: np.sqrt(sum([(b[i] - a[i]) ** 2 for i in range(len(a))]))
distance1 = lambda t: ditance(t[0], t[1])  # 1 variable
distance_point_to_plane = lambda point, a, b, c, d=0: abs(
    a * point[0] + b * point[1] + c * point[2] + d
) / np.sqrt(
    a ** 2 + b ** 2 + c ** 2
)  # point = (x1, y1, z1) & plane = ax + by + cz + d

# simple rotate
rotate2d = lambda a, b, teta: (
    np.cos(teta) * (a[0] - b[0]) - np.sin(teta) * (a[1] - b[1]) + b[0],
    np.sin(teta) * (a[0] - b[0]) + np.cos(teta) * (a[1] - b[1]) + b[1],
)

# straight lines
slope = lambda a, b: (b[1] - a[1]) / (b[0] - a[0])
line_equation = lambda a, b: lambda x: slope(a, b) * (x - a[0]) + a[1]
intersection = lambda x1, y1, x2, y2, x3, y3, x4, y4: [
...
triangle_surface3d = lambda a, b, c: triangle_surface3d_vect(
    vector.vector(a, b), vector.vector(a, c)
)
triangle_surface3d_vect = (
    lambda ab, ac: np.sqrt(
...
# triangles
centroid_of_triangle = lambda a, b, c: [
    (a[i] + b[i] + c[i]) / 3 for i in range(len(a))
]  # shoud work with any dim (except dim = -inf ?)
...
def get_x_axis_surface(triangle_strip):
    ...
    center_of_polygon = (
        min_y[middle_index][0] + (max_y[middle_index][0] - min_y[middle_index][0]) / 2,
        max_y[middle_index][1],
    )
    ...
    surface = sum(
        [
            triangle_surface2d(triangle[0], triangle[1], triangle[2])
...
def get_z_axis_surface(triangle_strip):
    ...
    for line in triangle_strip:
        ...
        max_y.append(
            max([coord.position[:2] for coord in line], key=lambda x: x[1])[::-1]
        )
        min_y.append(
            min([coord.position[:2] for coord in line], key=lambda x: x[1])[::-1]
        )
...
def no_x_before(
    node_index, triangle_strip, sign
):  # sign: 1 => begin to end, 2 => end to begin
...
def triangle_strip_dist_per_node(triangle_strip, diagonal=False, return_sum=False):
    ...
    for line in triangle_strip[:-1]:
        for node in line[:-1]:
            ...
            if diagonal:
                distances.append(sum([AB, AC, distance3d(B, C)]))
            else:
                distances.append(AB + AC)
...
def debug_triangle_strip(triangle_strip):
    ...
    try:
        ...
        raise TypeError(
            "the triangle_strip is not a strip anymore. It's type is {}".format(
                type(triangle_strip)
            )
        )
    ...
    try:
        ...
        raise TypeError(
            "the triangle_strip' is not are not lines anymore. Their type is {}".format(
                type(triangle_strip[0])
            )
        )

    assert all([type(node) == Node for node in [line for line in triangle_strip]])
    assert all(
        [hasattr(node, "position") for node in [line for line in triangle_strip]]
    )
    assert all(
        [hasattr(node, "connections") for node in [line for line in triangle_strip]]
    )
